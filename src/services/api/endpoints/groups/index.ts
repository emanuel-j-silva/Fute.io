...
export async function deleteGroup(groupId: string): Promise<RegisterResponse> {
    try {
        const response = await api.delete(`/groups/${groupId}`);
        return {
            message: response.data.message || "Grupo excluído com sucesso!",
            isError: false,
        };
    } catch (error: any) {
        if (error.response) {
            const errorMessage = error.response.data.errorMessage || "Erro desconhecido ao excluir grupo.";
            return {
                message: errorMessage,
                isError: true,
            };
